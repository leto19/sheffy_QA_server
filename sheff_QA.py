...
import watchdog
def fetch_archive_from_file(in_path: str, output_dir: str) -> bool:
    """
    ...
    _, _, archive_extension = in_path.rpartition(".")
    with open(in_path,"rb") as f:
        request_data =  f.read()

        if archive_extension == "zip":
            zip_archive = zipfile.ZipFile(io.BytesIO(request_data))
            zip_archive.extractall(output_dir)
        elif archive_extension == "gz" and not "tar.gz" in in_path:
            gzip_archive = gzip.GzipFile(fileobj=io.BytesIO(request_data))
            ...
            logger.warning(
                ...
                "See haystack documentation for support of more file types".format(in_path)
            )
...
def tutorial3_basic_qa_pipeline_without_elasticsearch():
    ...
    prediction = pipe.run(
        query="what is is the sheffield football team", params={"Retriever": {"top_k": 10}, "Reader": {"top_k": 5}}
    )
...
def answer_question(question,pipe):
   

    ## Voilà! Ask a question!
    prediction = pipe.run(
        query=question, params={"Retriever": {"top_k": 10}, "Reader": {"top_k": 2}}
    )
...
if __name__ == "__main__":
    ...
    answer_question("When was the University of Sheffield founded")
